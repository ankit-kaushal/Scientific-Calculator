...
public class Calculations {
	...
	private double evaluateMain (String expression) throws Exception {
		...
		for(int i=0; i<expression.length(); i++) {
			...
			if(expression.charAt(i)== 'e') {
				String sub_expr_r_= this.getVal(expression, i+1);
				String sub_res_r_= Math.exp(Double.parseDouble(sub_expr_r_))+"";
				expression= expression.replaceFirst(Pattern.quote("e" +sub_expr_r_), sub_res_r_+"");
				expression= this.remove_e(expression);
				i= -1;
			}
		}
			...
			if(expression.charAt(i)== 'b') {
				String sub_expr_r_= this.getVal(expression, i+1);
				String sub_res_r_= Math.abs(Double.parseDouble(sub_expr_r_))+"";
				expression= expression.replaceFirst(Pattern.quote("b" +sub_expr_r_), sub_res_r_+"");
				expression= this.remove_e(expression);
				i= -1;
			}
		}
		...
		for(int i=0; i<expression.length(); i++) {
			...
			if(expression.charAt(i)== 'i') {
				String tmp_sub_oper= this.getVal(expression, i+1);
				sub_expr= 'i'+ tmp_sub_oper;
				if(tmp_sub_oper.charAt(0)=='~')
					tmp_sub_oper= '-'+ tmp_sub_oper.substring(1);
				if(this.tFormula== 'd')
					sub_res= Math.sinh(Double.parseDouble(tmp_sub_oper)*Math.PI/180);
				else if(this.tFormula== 'r')
					sub_res= Math.sinh(Double.parseDouble(tmp_sub_oper));
				else if(this.tFormula== 'g')
					sub_res= Math.sinh(Double.parseDouble(tmp_sub_oper)*Math.PI/200);
				got_trig= true;
			}
			else if(expression.charAt(i)== 'j') {
				String tmp_sub_oper= this.getVal(expression, i+1);
				sub_expr= 'j'+ tmp_sub_oper;
				if(tmp_sub_oper.charAt(0)=='~')
					tmp_sub_oper= '-'+ tmp_sub_oper.substring(1);
				if(this.tFormula== 'd')
					sub_res= Math.cosh(Double.parseDouble(tmp_sub_oper)*Math.PI/180);
				else if(this.tFormula== 'r')
					sub_res= Math.cosh(Double.parseDouble(tmp_sub_oper));
				else if(this.tFormula== 'g')
					sub_res= Math.cosh(Double.parseDouble(tmp_sub_oper)*Math.PI/200);
				got_trig= true;
			}
			else if(expression.charAt(i)== 'k') {
				String tmp_sub_oper= this.getVal(expression, i+1);
				sub_expr= 'k'+ tmp_sub_oper;
				if(tmp_sub_oper.charAt(0)=='~')
					tmp_sub_oper= '-'+ tmp_sub_oper.substring(1);
				if(this.tFormula== 'd')
					sub_res= Math.tanh(Double.parseDouble(tmp_sub_oper)*Math.PI/180);
				else if(this.tFormula== 'r')
					sub_res= Math.tanh(Double.parseDouble(tmp_sub_oper));
				else if(this.tFormula== 'g')
					sub_res= Math.tanh(Double.parseDouble(tmp_sub_oper)*Math.PI/200);
				got_trig= true;
			}
		...
		for(int i=position+1; i<expr.length() && !end; i++) {
			...
			for(int j=0; j<trigOP.length && !end; j++) {
				if(trigOP[j]== expr.charAt(i)) {
                       ...
                       /* for(int j=0; j<trighOP.length && !end; j++) {
				if(trighOP[j]== expr.charAt(i)) {
					end= true;
				}
			}*/
			if(!end) {
	...
	private String formatExpression(String expr) {
		...
		expr= expr.replace("Tan", "t");
                //expr= expr.replace("Sinh", "w");
                
                //expr= expr.replace("SinH", "i");
